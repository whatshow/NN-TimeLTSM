...
sys.path.append("../..");
from TimeLSTM_v2 import TimeLSTM_v2 as TimeLSTM

# 1 feature
class RNN_LSTMAGV7:
    
    folder      = "rnn_lstm_ag_v7/"
    ...
    def __call__(self, path, device, lstm_layer_neuron_num, lstm_in_feature_num, time_step, epoch_iter, epoch_size, batch_size, learning_rate, data_train_x, data_train_y, data_test_x):
        ...
        criterion2 = nn.MSELoss(reduction='none').to(device);
        ...
        print("- RNN_LSTM_AlexGraves_V7 (100 epo)");
        # try to train
        if not has_trained:
            print("  - train");
            model.train();
            ...
            for epo_id in range(epoch_iter):
                ...
                for batch_id in range(epoch_size):
                    ...
                    loss2 = criterion2(out_tmp, data_train_y_cur);
                    # backward
